...
public class AccesoDatos {
	...
	private PreparedStatement getUsuarios;
	private PreparedStatement getNombreProyecto;
	...
	public AccesoDatos(Connection c) throws Exception {
		...
		getUsuarios=c.prepareStatement("select * from users");
		getNombreProyecto=c.prepareStatement("select * from proyectos where pr_id=?");
	...
	public ArrayList<Usuario> getAllUsuarios() throws Exception{
		ResultSet res=getUsuarios.executeQuery();
		res.beforeFirst();
		ArrayList<Usuario> usuarios=new ArrayList<Usuario>();
		while (res.next()){
			Usuario usr=new Usuario();
			usr.setId(res.getInt(1));
			usr.setNombre(res.getString(2));
	...
	public String nombreProyecto(int proyecto) throws Exception{
		getNombreProyecto.setInt(1, proyecto);
		ResultSet rs=getNombreProyecto.executeQuery();
		rs.first();
